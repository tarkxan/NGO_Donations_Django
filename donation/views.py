...
from donation.forms import SignUpForm
...
class UserRemove(UpdateView):
    model = DonationUser
    template_name = 'donation/user/remove.html'
    fields = ['is_active']

    def get_success_url(self):
        return reverse_lazy('donation:user_list')

    def get_context_data(self, *args, object_list=None, **kwargs):
        user = self.request.user
        context = super(UserRemove, self).get_context_data(*args, object_list=object_list, **kwargs)
        context['sidebar_urls'] = get_sidebar_urls(user)
        context['full_name'] = self.object.first_name + ' ' + self.object.last_name
        return context
...
class UserListEdit(UpdateView):
    ...
    def get_context_data(self, *args, object_list=None, **kwargs):
        user = self.request.user
        context = super(UserListEdit, self).get_context_data(*args, object_list=object_list, **kwargs)
        context['sidebar_urls'] = get_sidebar_urls(user)
        return context


def add_user(request):
    template_name = 'donation/user/add.html'
    context = {}
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('donation:user_list')
